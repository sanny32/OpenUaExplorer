# Fonts bundled by Qlementine 1.4.2

Qlementine 1.4.2 embeds the following font files in its static Qt resources.

## Inter and Inter Display

Version: 4.001 (git-9221beed3)

Copyright 2016 The Inter Project Authors.

License: SIL Open Font License 1.1. The license text is provided in
`OFL-1.1.txt`.

Inter UI and Inter are trademarks of rsms.

Source: https://github.com/rsms/inter

## Roboto Mono

Version: 3.000

Copyright 2015 The Roboto Mono Project Authors
(https://github.com/googlefonts/robotomono).

License: Apache License 2.0. The license text is provided in
`Apache-2.0.txt`.

Roboto Mono is a trademark of Google.

Source: https://github.com/googlefonts/robotomono
