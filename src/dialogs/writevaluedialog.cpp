...
QVariant WriteValueDialog::convertScalar(const QString &text, int type, bool *ok) const
{
    *ok = true;
    ...
    case Int32: return text.toInt(ok);
    case UInt32:
    case StatusCode: return text.toUInt(ok);
    case Int64: return text.toLongLong(ok);
    case UInt64: return text.toULongLong(ok);
    case Float: return text.toFloat(ok);
    case Double: return text.toDouble(ok);
    case DateTime: {
